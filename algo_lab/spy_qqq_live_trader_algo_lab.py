...
sys.path.insert(0, str(Path(__file__).parent.parent))  # quant_trading/ 루트
...
# ── 로거 ──────────────────────────────────────────────────────────────────────
LOG_DIR = Path(__file__).parent.parent / "logs" / date.today().strftime("%Y/%m/%d")
...
class SPYQQQLiveTrader:
    ...
    def _sync_with_broker(self):
        ...
        cash   = self.executor.get_account_cash()
    ...
    async def _maybe_process_bar(self, sym: str):
        ...
        # ── algo_lab hook D: long 포지션 보유 bar 수 누적 ─────────────────
    ...
    async def _enter(self, sym: str, sig: float, now: datetime):
        async with self._trade_locks[sym]:
            ...
            actual_entry = await asyncio.to_thread(self.executor.get_avg_entry_price, sym)
