#!/usr/bin/env python3
# ┌─────────────────────────────────────────────────────────────┐
# │  BACKTEST ARCHIVE  —  Version 1                             │
# │  Archived : 2026-05-29                                      │
# │  Origin   : backtesting/engine/backtest_spy_qqq.py          │
# │  Notes    : 원본 최초 아카이빙. 파라미터 수정 없음.         │
# └─────────────────────────────────────────────────────────────┘
"""
...
신호 계산 (벡터화):
  OBI : quotes → (bid_sz - ask_sz)/(bid_sz + ask_sz) → ewm(α=0.08)
  TFI : trades + Lee-Ready + 5분 롤링 → ewm(α=0.15)
  Signal = clip(OBI×0.35 + TFI×0.65, -1, 1) → 1분봉 마지막 값
...
CACHE_DIR = Path.home() / "quant_trading" / "logs" / "tick_cache"
...
def compute_signals(sym: str, date_str: str) -> pd.Series:
    ...
    obi = obi.reindex(idx, method="ffill").fillna(0.0)
    tfi = tfi.reindex(idx, method="ffill").fillna(0.0)
    sig = np.clip(obi * 0.35 + tfi * 0.65, -1.0, 1.0)
    return sig
...
def run_backtest(sym: str = "SPY",
                 ...
                 end:   str = "2026-05-15") -> pd.DataFrame:
    ...
    for date_str in days:
        ...
        day_trades, equity, daily_notional = simulate_day(
...
def _plot(df: pd.DataFrame, curve: pd.DataFrame, sym: str):
    ...
    fig.suptitle(f"{sym} OBI+TFI Strategy Backtest  |  "
                 f"2026-05-01 ~ 2026-05-15  |  ATR Dynamic Scaling + EOD Staggered Exit",
...
if __name__ == "__main__":
    run_backtest(sym="QQQ", start="2026-05-01", end="2026-05-27")
